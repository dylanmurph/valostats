    ...
    <div class="bg-background min-h-screen flex flex-col items-center py-10 px-4 sm:px-6">
        ...
        <div class="relative w-full max-w-[1500px] mb-10">
            ...
            <div class="rounded-t-xl bg-surface w-full min-h-[220px] sm:min-h-[260px] relative">
                <img src="{{ asset('images/maps/' . strtolower($bestMap?->name ?? 'abyss') . '.png') }}"
                     alt="Banner"
                ...
                <div class="absolute w-36 sm:w-44 aspect-square bg-surface rounded-full bottom-[-87px] left-4 sm:left-6 border-4 border-aqua shadow-lg z-10 overflow-hidden">
                    <img src="{{ asset('images/agents/' . strtolower($bestAgent?->name ?? 'brimstone') . '.png') }}"
                         alt="{{ $player->username }}"
            ...
            {{-- Nav Bar --}}
            <div class="bg-surface h-12 flex gap-2 pl-44 sm:pl-52 rounded-b-xl" id="tab-buttons">
        ...
        {{-- Overview Tab --}}
        <div id="tab-overview" class="tab-section bg-surface rounded-xl p-6 sm:p-10 w-full max-w-7xl mt-10 space-y-6">
                {{-- ELO Row --}}
                <div class="flex flex-col sm:flex-row gap-6">
                    <div class="flex-1 bg-mid rounded-xl p-4 flex items-center gap-5">
                        {{-- Rank Icon --}}
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {{-- Total Games --}}
                <div class="bg-mid rounded-xl p-4 flex flex-col justify-center">
        ...
        {{-- Agent Stats Tab --}}
        <div id="tab-agents" class="tab-section hidden bg-surface rounded-xl p-6 sm:p-10 w-full max-w-7xl mt-10 space-y-6">
            <h2 class="text-2xl font-bold text-aqua text-center">Agents</h2>

            @php
                $sortedAgentStats = $player->agentStats->sortByDesc('win_rate');
            @endphp

            <div class="space-y-4">
                @foreach ($sortedAgentStats as $stat)
                    <div class="relative bg-mid rounded-xl px-6 py-4 flex items-center shadow-md overflow-hidden min-h-[150px]">
                        ...
                        <div class="z-10 text-foreground w-2/3 space-y-4">
                            ...
                            {{-- Two-column stat grid aligned left --}}
        ...
        {{-- Map Stats Tab --}}
        <div id="tab-maps" class="tab-section hidden bg-surface rounded-xl p-6 sm:p-10 w-full max-w-7xl mt-10 space-y-6">
            <h2 class="text-2xl font-bold text-aqua text-center">Maps</h2>

            @php
                $sortedMapStats = $player->mapStats->sortByDesc('kda_ratio');
            @endphp
